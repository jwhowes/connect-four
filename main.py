...
def cli(ctx: click.Context, exp_dir: str):
    ...
    if not os.path.exists(train_config_path):
        ctx.obj["train_config"] = TrainConfig()
    else:
        ctx.obj["train_config"] = TrainConfig.from_yaml(train_config_path)
...
@click.option("--temperature", type=click.FloatRange(min=0.0, min_open=True), default=None)
